...
(function () {
    ...
    function initButtons() {
        ...
        dialog.onok = function () {
            var remote = false, list = [], id, tabs = $('#tabhead').children;
        ...
        initEvents: function () {
            ...
            domUtils.on($G("width"), 'keyup', function(){
                ...
                if(locker.checked) {
                    ...
                    $('#height').value = Math.round(this.value / proportion);
                } else {
                    _this.updateLocker();
                }
            });
            domUtils.on($G("height"), 'keyup', function(){
                ...
                if(locker.checked) {
                    var proportion =locker.getAttribute('data-proportion');
                    $('#width').value = Math.round(this.value * proportion);
                } else {
                    _this.updateLocker();
                }
            });
    ...
    OnlineImage.prototype = {
        ...
        initEvents: function () {
            ...
            /* 滚动拉取图片 */
            domUtils.on($('#imageList'), 'scroll', function(e){
                var panel = this;
    ...
    SearchImage.prototype = {
        ...
        initEvents: function(){
            ...
            /* 点击搜索按钮 */
            domUtils.on($('#searchBtn'), 'click', function(){
                var key = $('#searchTxt').value;
                if(key && key != lang.searchRemind) {
                    _this.getImageData();
                }
            });
            /* 点击清除妞 */
            domUtils.on($('#searchReset'), 'click', function(){
                $('#searchTxt').value = lang.searchRemind;
                $('#searchListUl').innerHTML = '';
                $('#searchType').selectedIndex = 0;
            });
            /* 搜索框聚焦 */
            domUtils.on($('#searchTxt'), 'focus', function(){
                var key = $('#searchTxt').value;
                if(key && key == lang.searchRemind) {
                    $('#searchTxt').value = '';
                }
            ...
            /* 搜索框回车键搜索 */
            domUtils.on($('#searchTxt'), 'keydown', function(e){
            ...
            /* 选中图片 */
            domUtils.on($('#searchList'), 'click', function(e){
                var target = e.target || e.srcElement,
        ...
        getImageData: function(){
            ...
            $('#searchListUl').innerHTML = lang.searchLoading;
            ajax.request(url, {
                'dataType': 'jsonp',
                'charset': 'GB18030',
                ...
                'onerror':function(){
                    $('#searchListUl').innerHTML = lang.searchRetry;
                }
            });
        },
        /* 添加图片到列表界面上 */
