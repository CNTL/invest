...
!function($) {
   ...
   $.fn.validation.defaults = {
    ...
    var validateField = function(field, valid) { 
        ...
        //增加验证成功后的对号,help-success是一个空类只做选择器使用
        if (controlGroup.hasClass("has-success")) {
            //处理横向表单 打对号, 基本表单和内联表单不需要打对号，需要可以自己扩展
            if (isformstyle(el.closest("form")) == 2) { //横向表单
                controlGroup.find(".help-success").remove();
                el.parent().after('<span class="help-success" style="color:#3C763D;font-weight:bold;">√</span>');
        ...
        if (globalOptions.reqmark==true){
            if(fform_style==0){
                $(obj).find(".form-group>label").each(function(){
                    ...
                    el.after('<span id="autoreqmark" style="color:#FF9966;font-weight:bold;"> *</span>')
                });
            }
