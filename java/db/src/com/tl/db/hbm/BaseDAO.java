...
import com.tl.db.cfg.QueryOrder;
import com.tl.db.cfg.QueryOrder.OrderRule;
import com.tl.db.cfg.QueryParameter;
...
public abstract class BaseDAO {
	 ...
	 */
	protected static synchronized Configuration init(String configFile)throws HibernateException {		
		//若配置文件空
	 ...
	 * @return Configuration 返回配置对象，子类中可能有用
	 */
	protected static synchronized Configuration init(File configFile)throws HibernateException {		
	 ...
	 * @return a distinct list of instances (or arrays of instances)
	 */
	public java.util.List find(String queryStr, int start, int length) throws HibernateException {
		Session s = null;
		try {
			s = getSession();
	 ...
	 * @return a distinct list of instances (or arrays of instances)
	 */
	public java.util.List find(String query, QueryParameter parameter) throws HibernateException {
		Session s = null;
		try {
			s = getSession();
			return find(query, parameter, s);
		}
		finally {
			closeSession(s);
		}
	 ...
	 * @param s the Session to use
	 */
	public java.util.List find(String query, QueryParameter parameter, Session s) throws HibernateException {
		Query q = s.createQuery(query);
		return q.setParameter(parameter.getName(), parameter.getValue(), parameter.getType())
			.list();
	}
	 ...
	 * 
	 * @param query HQL查找语句
	 * @param parameters HQL语句中的参数数组
	 * @param s the Session to use
	 */
	public List find(String query, QueryParameter[] parameters, Session s) throws HibernateException {
		Query q = s.createQuery(query);
		for (int i = 0; i < parameters.length; i++)
			q.setParameter(parameters[i].getName(), parameters[i].getValue(), parameters[i].getType());
		return q.list();
	}
	 ...
	 * @throws HibernateException
	 */
	public java.util.List findAll (Class clazz, QueryOrder orderRule) throws HibernateException {
		Session s = null;
		try {
			s = getSession();
			return findAll(clazz, orderRule, s);
		}
		finally {
			closeSession(s);
		}
	 ...
	 * @throws HibernateException
	 */
	public java.util.List findAll (Class clazz,  QueryOrder orderRule, Session s) throws HibernateException {
		Criteria crit = createCriteria(s, clazz);
		if (null != orderRule) {
			String orderField = orderRule.getOrderField();
			OrderRule rule = orderRule.getOrderRule();
			if(StringUtils.isNotEmpty(orderField))
				crit.addOrder(rule == OrderRule.ASC ? Order.asc(orderField) : Order.desc(orderField));
		}
		return crit.list();
	}
	 ...
	 * @throws HibernateException
	 */
	public java.util.List findAll (Class clazz, QueryOrder[] orderRules) throws HibernateException {
		Session s = null;
		try {
			s = getSession();
			return findAll(clazz, orderRules, s);
		}
		finally {
			closeSession(s);
		}
	 ...
	 * @throws HibernateException
	 */
	public java.util.List findAll (Class clazz, QueryOrder[] orderRules, Session s) throws HibernateException {
		Criteria crit = createCriteria(s, clazz);
		if (null != orderRules) 
		{
			for (int i = 0; i < orderRules.length; i++)
			{
				String orderField = orderRules[i].getOrderField();
				OrderRule rule = orderRules[i].getOrderRule();
				if(StringUtils.isNotEmpty(orderField))
					crit.addOrder(rule == OrderRule.ASC ? Order.asc(orderField) : Order.desc(orderField));
			}
		}
	 ...
	 * @throws HibernateException
	 */
	public int delete (String query, QueryParameter parameter) throws HibernateException {
		Transaction t = null;
		Session s = null;
		try {
			s = getSession();
			t = beginTransaction(s);
			int result = delete(query, parameter, s);
			commitTransaction(t);
			return result;
		}
		catch (HibernateException e) {
	 ...
	 * @throws HibernateException
	 */
	public int delete (String query, QueryParameter parameter, Session s) throws HibernateException {
		Query q = s.createQuery(query);
		return q.setParameter(parameter.getName(), parameter.getValue(), parameter.getType())
			.executeUpdate();
	}
	...
	public int delete (String query, QueryParameter[] parameter, Session s) throws HibernateException {
		...
		for (int i = 0; i < parameter.length; i++)
			q.setParameter(parameter[i].getName(), parameter[i].getValue(), parameter[i].getType());
