...
public class Entry extends BaseController {
	...
	protected Menu[] getMainMenus(int mainType) {
		...
		list.add(new Menu(5, "机构","/user/CompanyMain.do?mainType=5",class5, -999,0));
	...
	*/ 
	protected Menu[] getPersonMenus(HttpServletRequest request) {
		...
		if(type == 0){//个人用户登录后
			...
			list.add(new Menu(3, "职位管理", "/resume/myrecruit.do?a=myrecruit&infoType=1","spo",-999,0));
			//list.add(new Menu(4, "简历管理", "/resume/myrecruit.do?a=myrecruit&infoType=2","spo",-999,0));
